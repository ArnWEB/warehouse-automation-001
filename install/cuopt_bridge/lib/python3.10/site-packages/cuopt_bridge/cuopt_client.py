...
class CuOptClient(Node):
    ...
    def _solve_cuopt(self):
        try:
            ...
            #     order_array_idx = row.get("order_array_index", -1)

            #     if truck_id < len(robot_ids):
            #         robot_id = robot_ids[truck_id]
            ...
            #         if row["location"] not in assignments[robot_id]["route"]:
            #             assignments[robot_id]["route"].append(int(row["location"]))

            #         if location_type == "Pickup" and order_array_idx >= 0:
            #             if order_array_idx not in assignments[robot_id]["tasks"]:
            #                 assignments[robot_id]["tasks"].append(order_array_idx)
            #                 order_mapping[str(order_array_idx)] = robot_id
            #         elif location_type == "Delivery":
            #             task_idx = order_array_idx - n_orders
            #             if task_idx >= 0 and task_idx not in assignments[robot_id]["tasks"]:
            #                 assignments[robot_id]["tasks"].append(task_idx)
            #                 order_mapping[str(task_idx)] = robot_id

            # for robot_id in assignments:
            #     assignments[robot_id]["total_cost"] = solution_cost / max(len(assignments), 1)
            #     assignments[robot_id]["estimated_time"] = len(assignments[robot_id]["tasks"]) * 2.5
            #     assignments[robot_id]["route"] = assignments[robot_id].get("route", [])
